  ...
  var LoadScriptHelper = (function() {
    ...
    LoadScriptHelper.prototype.loadScriptInternal = function(url, scriptId, callback, highPriority, timeoutInMs) {
      if (url) {
        ...
        var loadedScriptEntry =
          scriptId && this.loadedScriptByIds[scriptId] ? this.loadedScriptByIds[scriptId] : this.getScriptEntryByUrl(url);
        if (!loadedScriptEntry || loadedScriptEntry.hasError || loadedScriptEntry.url.toLowerCase() != url.toLowerCase()) {
          ...
          var onLoadCallback = function OSF_OUtil_loadScript$onLoadCallback() {
            if (
              ...
              typeof OSF.AppTelemetry !== 'undefined' &&
              typeof OSF.AppTelemetry.enableTelemetry !== 'undefined'
            ) {
        ...
        for (var i = 0; i < this.scriptTelemetryBuffer.length; i++) {
          ...
          if (OSF.AppTelemetry.onScriptDone.length == 3) {
            ...
            OSF.AppTelemetry.onScriptDone(
              scriptTelemetry.scriptId,
              scriptTelemetry.startTime,
              scriptTelemetry.msResponseTime,
              this.osfControlAppCorrelationId
            );
          }
        }
...
OSF._OfficeAppFactory = (function OSF__OfficeAppFactory() {
  ...
  var _retrieveHostInfo = function OSF__OfficeAppFactory$_retrieveHostInfo() {
    ...
    if (!hostInfoValue) {
      try {
        ...
        if (fallbackHostInfo == 'isDialog') {
          ...
          fallbackHostInfo.toLowerCase().indexOf('mac') !== -1 &&
          fallbackHostInfo.toLowerCase().indexOf('outlook') !== -1 &&
          shouldLoadOldMacJs()
        ) {
          _hostInfo.isO15 = true;
        } else {
    ...
    if (hostInfoValue) {
      ...
      _hostInfo.isDialog = typeof items[5] != 'undefined' && items[5] == 'isDialog';
      ...
      if (
        ...
        OSF.HostSpecificFileVersionMap[_hostInfo.hostType][_hostInfo.hostPlatform]
      ) {
  ...
  var initialize = function OSF__OfficeAppFactory$initialize() {
    ...
    var isMicrosftAjaxLoaded = function OSF$isMicrosftAjaxLoaded() {
      if (
        (typeof Sys !== 'undefined' &&
          ...
          Sys.StringBuilder &&
          typeof Sys.StringBuilder === 'function' &&
          Type.registerNamespace &&
          typeof Type.registerNamespace === 'function' &&
          Type.registerClass &&
          typeof Type.registerClass === 'function') ||
    ...
    var loadLocaleStrings = function OSF__OfficeAppFactory_initialize$loadLocaleStrings(appLocale) {
      ...
      var localeStringFile =
        basePath + OSF.getSupportedLocale(appLocale, OSF.ConstantNames.DefaultLocale) + '/' + OSF.ConstantNames.OfficeStringJS;
      _loadScriptHelper.loadScript(
        localeStringFile,
        OSF.ConstantNames.OfficeStringsId,
        loadLocaleStringCallback,
    ...
    if (_hostInfo.isO15) {
      ...
      hostSpecificFileName = _hostInfo.hostType + '-' + 'sdx' + '-' + _hostInfo.hostSpecificFileVersion + '.debug.js';
    ...
    if (requiresMsAjax && !isMicrosftAjaxLoaded()) {
      var msAjaxCDNPath =
        (window.location.protocol.toLowerCase() === 'https:' ? 'https:' : 'http:') + '//ajax.aspnetcdn.com/ajax/3.5/MicrosoftAjax.js';
  ...
  return {
    ...
    getHostInfo: function OSF_OfficeAppFactory$getHostInfo() {
      return _hostInfo;
    },
    ...
    getWindowName: function OSF__OfficeAppFactory$getWindowName() {
      return _windowName;
    }
  };
})();
